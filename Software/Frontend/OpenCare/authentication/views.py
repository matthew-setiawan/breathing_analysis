...
def sensorpost(request,data):
      values = data.split("_")
      if len(values)==4:
            device = values[0]
            heartrate = float(values[1])
            temp = float(values[2])
            acc = float(values[3])
            extfunctions.updatereading(device,"heartratereading",heartrate)
            extfunctions.updatereading(device,"temperaturereading",temp)
            extfunctions.updatereading(device,"accelerometerreading",acc)
      elif len(values)==2:
            device = values[0]
            ppm = float(values[1])
            extfunctions.updatereading(device,"co2reading",ppm)
      elif len(values)==3:
            device = values[0]
            area1 = float(values[1])
            area2 = float(values[2])
            extfunctions.updatereading(device,"knittedcoilarea1",area1)
            extfunctions.updatereading(device,"knittedcoilarea2",area2)
      elif len(values)==5:
            device = values[0]
            heartrate = float(values[1])
            temp = float(values[2])
            acc = float(values[3])
            ppm = float(values[4])
            extfunctions.updatereading(device,"heartratereadingprediction",heartrate)
            extfunctions.updatereading(device,"temperaturereadingprediction",temp)
            extfunctions.updatereading(device,"accelerometerreadingprediction",acc)
            extfunctions.updatereading(device,"co2readingprediction",ppm)
            
      return HttpResponse("200", content_type="text/html")
